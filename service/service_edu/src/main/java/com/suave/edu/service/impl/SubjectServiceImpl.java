...
import com.suave.edu.entity.subject.TwoSubject;
...
public class SubjectServiceImpl extends ServiceImpl<SubjectMapper, Subject> implements SubjectService {
    ...
    public List<OneSubject> getAllOneTwoSubject() {
        ...
        for (int i = 0; i < SubjectList1.size(); i++) { //遍历SubjectList1集合
            ...
            Subject subject = SubjectList1.get(i);
            ...
            for (int m = 0; m < SubjectList2.size(); m++) {
                //获取每个二级分类
                Subject eduSubject2 = SubjectList2.get(m);
                //判断二级分类parent_id和一级分类id是否一样  是否是一级分类下的分类
                if (eduSubject2.getParentId().equals(subject.getId())) {
